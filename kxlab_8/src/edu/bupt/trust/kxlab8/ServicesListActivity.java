...
import edu.bupt.trust.kxlab8.ServicesListFragment.OnServiceSelectedListener;
...
public class ServicesListActivity extends BaseActivity implements OnServiceSelectedListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		findViewById(R.id.footer_services).setEnabled(false);
	    ...
	    for(int i = 0; i < mFragmentTags.length; i++){
	    	...
	    	if(existingFragment == null){
				...
				actionBar.addTab(actionBar.newTab().setText(tabTitles[i]).setTabListener(
						new TabListener<ServicesListFragment>(this, mFragmentTags[i], ServicesListFragment.class)));
	    	} else {
	    		// if it does it exist, use that fragment as the tab content
				actionBar.addTab(actionBar.newTab().setText(tabTitles[i]).setTabListener(
						new TabListener<ServicesListFragment>(this, mFragmentTags[i], existingFragment)));
	    	}
	    }
